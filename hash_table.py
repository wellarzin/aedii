...
nomes = [
    ...
    "Roberta", "Roberto", "Carla", "Karl", "Marcos", "Marcus", "Pablo",
    "Pabllo", "Fernanda", "Anne", "Ane", "Joao", "Ana", "Juliano", "Josias", 
    "Alberto"
]
...
class HashTable():
  ...
  def avaliar_hash(self):
    ...
    print(
        f"Posições ocupadas:              {posicoes_ocupadas} ({posicoes_ocupadas/self.size*100:.1f}%)"
    )
    print(
        f"Posições vazias:                {posicoes_vazias} ({posicoes_vazias/self.size*100:.1f}%)"
    )
    print(f"-" * 60)
